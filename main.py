...
from data import ModelNet40, Kitti2015Reg, Kitti2015Flow
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='modelnet40', choices=['modelnet40', 'kitti2015reg', 'kitti2015flow'], metavar='N',
                        help='dataset to use')
    ...
    if args.dataset == 'modelnet40':
        ...
        test_loader = DataLoader(
            Kitti2015Reg(num_points=args.num_points, partition='test', gaussian_noise=args.gaussian_noise,
                       unseen=args.unseen, factor=args.factor),
            batch_size=args.test_batch_size, shuffle=False, drop_last=False)    
    elif args.dataset == 'kitti2015flow':
        train_loader = DataLoader(
            Kitti2015Flow(num_points=args.num_points, partition='train', gaussian_noise=args.gaussian_noise,
                       unseen=args.unseen, factor=args.factor),
            batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(
            Kitti2015Flow(num_points=args.num_points, partition='test', gaussian_noise=args.gaussian_noise,
                       unseen=args.unseen, factor=args.factor),
            batch_size=args.test_batch_size, shuffle=False, drop_last=False) 
    else:

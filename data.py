...
#pc1, pc2, gt
def load_kitti_flow_data(partition):
    download()
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'kitti_data')
    all_pc1 = []
    all_pc2 = []
    all_gt = []
    for h5_name in glob.glob(os.path.join(DATA_DIR, 'kitti_scene_flow_%s*.h5' % partition)):
        f = h5py.File(h5_name)
...
class Kitti2015Flow(Dataset):
    def __init__(self, num_points, partition='train', gaussian_noise=False, unseen=False, factor=4):
        print('----- Loading KITTI 2015 Scene Flow Dataset -----')
        self.pc1, self.pc2, self.gt = load_kitti_flow_data(partition)
        self.num_points = num_points
        self.partition = partition
        self.gaussian_noise = gaussian_noise
        self.unseen = unseen
    ...
    def __getitem__(self, item):
        ...
        return pointcloud1.astype('float32'), pointcloud2.astype('float32'), gt_flow.astype('float32')

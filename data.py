...
class SceneFlow(Dataset):
    def __init__(self, dataset_name, num_points, partition='train', gaussian_noise=False, unseen=False, factor=4):
        if dataset_name == "flyingthings3dflow":
            dataset_name = 'flyingthings3d'
        elif dataset_name == "kitti2015flow":
            dataset_name = 'kitti'
        
        print('----- Loading %s Scene Flow Dataset -----'%(dataset_name.upper()))
        self.pc1, self.pc2, self.gt = load_scene_flow_data(dataset_name, partition)
        self.num_points = num_points
        self.partition = partition
        self.gaussian_noise = gaussian_noise
        self.unseen = unseen
